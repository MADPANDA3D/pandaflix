...
| `sub_speed` | `1` | Subtitle timing scale (mpv `--sub-speed`); below 1 slows subtitles that run ahead (e.g. `0.96` for a 23.976/25 fps mismatch) |
| `subtitle_source` | `"external"` | `"external"` prefers OpenSubtitles (consistent timing), `"stream"` prefers provider-embedded subtitles, `"auto"` uses streams when present |
